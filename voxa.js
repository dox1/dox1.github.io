...
async function generateImage(prompt) {
  try {
    ...
    const data = await response.json();
    // نبحث عن الجزء الذي يحتوي على الصورة المشفرة بـ base64
    const imagePart = data.candidates[0].content.parts.find(p => p.inlineData);
    return imagePart.inlineData.data; // سترجع سلسلة base64
  } catch (err) {
    console.error(err);
    return null;
  }
...
// ======================
// 2. دالة تعديل صورة موجودة
// ======================
async function editImage(originalBase64, editPrompt) {
  try {
    const response = await fetch(
      `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash-preview-image-generation:generateContent?key=${GEMINI_API_KEY}`,
      {
...
document.getElementById('genImageBtn').addEventListener('click', async () => {
  ...
  if (b64) {
    document.getElementById('generatedImage').src = `data:image/png;base64,${b64}`;
  }
});
...
document.getElementById('editImageBtn').addEventListener('click', async () => {
  ...
  if (fileInput.files.length === 0) return alert('اختر صورة أولاً');
  const file = fileInput.files[0];
  const reader = new FileReader();
  reader.onload = async () => {
    const base64 = reader.result.split(',')[1];
    const editedB64 = await editImage(base64, prompt);
    if (editedB64) {
      document.getElementById('editedImage').src = `data:image/png;base64,${editedB64}`;
    }
  };
